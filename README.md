...
● ポートフォリオ（バックエンド側）　https://github.com/ryoichi-k/ex_sns

● 趣味：筋トレ、プログラミング、読書、料理、日帰り旅行、イラスト

● この先やってみたいこと：技術同人誌を製作したい。将来の技術書出版のためにテックブログを開設し、ライティングスキルを磨きたい。

　　○ Rubyのエラーをまとめた同人誌がとてもわかりやすく、それに影響を受けて技術書の出版をしたいと思うようになりました。
  
　　○ ２、３年後にテックブログを開設したい。それまでは自分のスキルの向上や様々なことの体験に当てたい。

● 自社サービスを保有する会社でプロダクトを成長させる経験をしたい。

　　○ プロダクトの方針についてジャッジできるようなテックリードになりたい。なんらかの言語のコミッターになりたい。（GOなどを検討中）
...
● 現職に入社してから、宿泊サイト、ナイトレジャー検索サイト、買取専門ECサイト、輸入雑貨ECサイトの開発案件に従事し、フロントエンドとバックエンドの両方を担ってきました。
...
　　○ 基本設計～テスト（基本設計書、単体テスト仕様書、データベース定義書の作成経験あり）
  
 　　○ Backlogでタスクを管理、取り組むというケースが多かった。
   
 　　○ zoomミーティング、Slackのハドルミーティングを駆使しての打ち合わせ、技術的な質問をしての業務経験あり。（オンラインでのコミュニケーションも得意です。）
...
| 言語環境 | 期間 | レベル |
|:------|:------:|:------:|
| PHP | 1年以上 | PHP7、8での開発経験あり。基本設計経験あり。指示通り作業が可能 |
| Python3 | 半年以上 | 基本設計経験あり。指示通り作業が可能。Pandas使用経験あり |
| Javascript | 1年以上 | 基本設計経験あり。調べながら構築可能。ES2015で書いた経験あり。 アロー関数、async、await使えます。|
| Laravel | 半年 | 基本設計経験あり。調べながら構築可能、Laravel9の経験あり。migrationファイル、seeder、trait作成経験あり。 |
| Vue.js | 6ヶ月 | 基本設計経験あり。調べながら構築可能、ElementPlusの経験あり。propsでコンポーネントに値を渡す機能の使用経験あり。 |
...
● 大学院科目履修生として情報アーキテクチャ学を聴講経験

● 自己学習の言語と期間

| 言語 | 期間 | レベル |
|:------|:------:|:------:|
| Node.js | ６ヶ月以上 | REST APIを自作できる、Expressについて基礎を理解している、mongooseを使用できる、postmanでのAPIのテストができる。MySQLとともに使用したこともある。 |
...
| AWS資格試験 | ６ヶ月以上 |  ソリューションアーキテクトアソシエイト試験勉強　１２月に受験予定。クラウドプラクティショナーには合格。connpassのAWSハンズオンイベントが印象に残っておすすめ |
| lambda | 数日 | pythonで簡単な関数を作ってcloudwatchで監視。API Gatewayも通してみた。「DynamoDB、RDSと繋げば、これだけでバックエンドになるやん！」とテンションが上がる。考えた人は天才。 |
| LINE Messaging API | 数日 | 興味本位で触ってみた程度。無料プランでは月に1000件しかメッセージ送れないのは辛いと考えていたが、別にそうでもなかった。 |
| wireshark | 3ヶ月 | 大学院の講義がきっかけで学習。パケットの流れが少しわかる程度。不正なパケットを見抜く講義が面白かった。 |
| Go | 1ヶ月 | 『詳解Go言語Webアプリケーション開発』　著者：清水陽一郎　を読んで学習。この言語に可能性を感じている自分がいる。 |
| CakePHP | 1ヶ月以上 | 随分前だが、掲示板アプリを自作した。Laravelの案件に参画できてからは触らなくなった。公式サイトのデザインが好き。 |
| AtCoder | 2ヶ月以上 | アルゴリズムの学習は面白い。|
...
※経験しているスキルセットはこちらをご覧ください！resumeのサイトに遷移します。　⇨：https://www.resume.id/ryo_k
...
・新しい技術はガンガン触っています！技術書を読むのが好きでいつも大型の本屋に通っています。（池袋のジュンク堂書店がお気に入り）
